...
//* Scene container
    ...
    Renderer.prototype.render = function (scene, camera, nodes) {
        ...
        if (this._debug)
            scene.root.getAllChildren(nodes);
...
// this._bounding not added in the definition
    ...
    function LightNode(options) {
        this._ctor();
        this._render_priority = 30;
        this._texture =  GL.Texture.fromURL( "../img/translate.png", {filter: gl.NEAREST, wrap: gl.REPEAT});;
    }

    extendClass(LightNode, SceneNode);
    global.LightNode = RD.LightNode = LightNode; // we set lightnode into all the namespaces
    // TODO fix the renderToQuad
    LightNode.prototype.render = function (renderer, camera) {
        temp_vec3 = this.getGlobalPosition(); // pos
        temp_vec3 = camera.project(temp_vec3); // screen pos
        this._texture.renderQuad( temp_vec3[0], temp_vec3[1], this._texture.width , this._texture.height);
    }
//footer
