...
//Scene Node
    ...
    Object.defineProperty(SceneNode.prototype, 'scene', {
        ...
        set: function (v) {
            throw("cannot set scene, add to root node");
        },
        enumerable: true //avoid problems
    });


// vik disabled because the property and the action sound the same
    ...
    Object.defineProperty(SceneNode.prototype, 'parentNode', {
        ...
        set: function (v) {
            throw("Cannot set parentNode of GameObject");
        },
        enumerable: false //avoid problems
    });
...
//recursively retrieves all children nodes
    SceneNode.prototype.getAllChildren = function (r, lights) {
        r = r || [];
        lights = lights || [];
        for (var i = 0, l = this.children.length; i < l; i++) {
            var node = this.children[i];
            if (node.constructor.name == "LightNode")
                lights.push(node);
            r.push(node);
            node.getAllChildren(r, lights);
        }
...
//vik instead of scale
    SceneNode.prototype.scaleFromVector = function (v) {
...
// vik
    SceneNode.prototype.setRotationFromEuler = function (v) {
...
//fast skips recomputation of parent, use it only if you are sure its already updated
    ...
    // vik
    SceneNode.prototype.getRootRecursively = function () {
        var root = this;
        var n_root = this;
        do {
            root = n_root;
...
// vik
    //transforming
    SceneNode.prototype.move = function (v) {
        vec3.add(this._position, this._position, v);
        this._must_update_matrix = true;
    }

    // vik
    Renderer.prototype.toggleDebug = function () {
        this._debug = !this._debug;
    }

    SceneNode.prototype.getScaleFactors = function () {
...
// this._bounding not added in the definition
    SceneNode.prototype.createBounding = function () {
        try {
            if (this.mesh) {
                var scale_factors = this.getScaleFactors();
                this._bounding = new RD.SceneNode();
                this._bounding.id = "bounding" + this.id;
                this._bounding.mesh = "bounding";
                this._bounding.primitive = gl.LINES;
                this._bounding.color = [0.3, 0.7, 0.56];
                this._bounding._scale.set(scale_factors);
                this._bounding.updateLocalMatrix();
                this.addChild(this._bounding);
            }
        } catch (e) {
        }
    }

    SceneNode.prototype.removeBounding = function () {
        if (this._bounding) {
            this.removeChild(this._bounding);
            this._bounding = null;
        }
    ...
    Billboard.prototype.render = function (renderer, camera) {
        ...
        if (this.auto_orient) {
            if (this.parallel) {
                ...
                mat4.lookAt(this._global_matrix, this.getGlobalPosition(), camera.position, RD.UP); // BUG position should be the global one
...
//* Scene container
    ...
    Scene.prototype.update = function (dt) {
        this.time += dt;
        this.root.propagate("update", [dt], false);
    }
    ...
    Object.defineProperty(Scene.prototype, 'root', {
        ...
        set: function (v) {
            throw("Cannot set root of scene");
        },
        enumerable: false //avoid problems
    });
    ...
    function Renderer(context) {
        ...
        //vik
        this._debug = false;
        this._prev_debug_state = this._debug;
    ...
    Renderer.prototype.render = function (scene, camera, nodes) {
        ...
        if (!nodes)
            scene.root.getAllChildren(this._nodes, this._lights);
        ...
        for (var i = 0; i < nodes.length; ++i) {
            ...
            if (this._debug && !this._prev_debug_state)//vik
                node.createBounding();
        ...
        for (var j = 0; j < this._lights.length; ++j) { // vik
            //rendering

            this._rendering_light = this._lights[j];
            this._rendering_light.updateLightVector();
            for (var i = 0; i < nodes.length; ++i) {
                if (j == 1) {
                    gl.enable(gl.BLEND);
...
//this functions allow to interrupt the render of one scene to render another one
    Renderer.prototype.saveState = function () {
        var nodes = [];
        this._nodes[0].scene.root.getAllChildren(nodes);
        var cam = this._camera.clone();
        var state = {
            camera: cam,
            nodes: nodes
        };

        this._state.push(state); // BUG should be _state not state
    }

    Renderer.prototype.restoreState = function () {
        var state = this._state.pop(); // BUG should be _state
        var camera = this._camera = state.camera;
//        this._view_matrix.set(camera._view_matrix);
//        this._projection_matrix.set(camera._projection_matrix);
//        this._viewprojection_matrix.set(camera._viewprojection_matrix);
//        this._uniforms.u_camera_position = camera.position;
        this._nodes = state.nodes;
        this._nodes[0].scene._root = this._nodes[0].getRootRecursively(); // vik
    }
    ...
    Renderer.prototype.renderNode = function (node, camera) {
        ...
        shader.uniforms({ u_lightvector: this._rendering_light._light_vector});
        shader.uniforms({ u_lightpos: this._rendering_light.getGlobalPosition()});
...
//box in {center:vec3,halfsize:vec3} format
	...
	');
        gl.shaders["texture"] = this._texture_shader; // BUG
        ...
        this._phong_shader = new GL.Shader('\
			...
			void main() {\
				...
				v_lightvector = u_lightpos - pos.xyz;\
				gl_Position =  u_mvp * vec4(a_vertex,1.0);\
			}\
			', '\
			precision highp float;\
			...
			void main() {\
			  ...
			  gl_FragColor = u_color * max(0.0, dot(L,N)) * vec4(u_lightcolor,1.0);\
			}\
		');
        ...
        this._textured_phong_shader = new GL.Shader('\
			...
			void main() {\n\
				...
				v_lightvector = pos.xyz - u_lightvector;\
				gl_Position = pos;\n\
			}\
			', '\
			precision highp float;\
...
//////////////// ext vik
    ...
    Camera.prototype.clone = function () {
        var out = new RD.Camera();
        out.type = this.type;
        out._position = vec3.clone(this._position);
        out._target = vec3.clone(this._target);
        out._up = vec3.clone(this._up);

        out.near = this.near;
        out.far = this.far;
        out.aspect = this.aspect;
        out.fov = this.fov; //persp
        out.frustum_size = this.frustum_size; //ortho


        out._view_matrix = mat4.clone(this._view_matrix);
        out._projection_matrix = mat4.clone(this._projection_matrix);
        out._viewprojection_matrix = mat4.clone(this._viewprojection_matrix);
        out._model_matrix = mat4.clone(this._model_matrix); //inverse of view

        out._must_update_matrix = this._must_update_matrix;

        out._top = vec3.clone(this._top);
        out._right = vec3.clone(this._right);
        out._front = vec3.clone(this._front);
        return out;
    }
    ...
    function LightNode() {
        ...
        this._ctor();
        this._target = vec3.fromValues(0, 0, 0);
        this._light_vector = vec3.fromValues(0, 0, 0);

        this._render_priority = RD.PRIORITY_HUD;
        this.mesh = "box";
        this.flags.two_sided = true;
        this.flags.visible = false;
        this.shader = "flat";

        var bb = new RD.Billboard();
        bb.mesh = "thin_plane";
        bb.id = "light_icon";
        bb.position = [0, 0, 0];
        bb.unselectable = true;
        bb.setTexture("color", "light");
        this.addChild(bb);

    }

    //extendClass(LightNode, Billboard);
    extendClass(LightNode, SceneNode);


    LightNode.prototype.updateLightVector = function () {
        vec3.sub(this._light_vector, this.getGlobalPosition(), this._target);
        vec3.normalize(this._light_vector, this._light_vector);
    }

    global.LightNode = RD.LightNode = LightNode; // we set lightnode into all the namespaces
