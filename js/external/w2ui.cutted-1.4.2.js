...
(function () {
    ...
    /* @const */
    var w2layout_panels = ['top', 'left', 'main', 'preview', 'right', 'bottom', 'lower_left', 'lower_right'];
    ...
    w2layout.prototype = {
        ...
        show: function (panel, immediate) {
            ...
            if (immediate === true) {
                ...
                $('#layout_' + obj.name + '_resizer_' + panel).show();
        ...
        render: function (box) {
            ...
            function resizeStart(type, evnt) {
                ...
                if (type == 'top' || type == 'preview' || type == 'bottom' || type == 'lower_left' || type == 'lower_right') {
                    obj.tmp.resize.value = parseInt($('#layout_' + obj.name + '_resizer_' + type)[0].style.top);
                }
            }

            // workaround continous update
            function resizeStop(evnt) {
                ...
                if (obj.tmp.diff_x !== 0 || obj.tmp.resize.diff_y !== 0) { // only recalculate if changed
                    ...
                    var pleft = obj.get('left');
                    var pright = obj.get('right');
            ...
            function resizeMoveUpdate(evnt) {
                ...
                switch (tmp.type) {
                    ...
                    case 'lower_right':
                    case 'lower_left':
                        var upper_panel = (tmp.type === 'lower_right') ? obj.get('right') : obj.get('left');
                        if (panel.minSize + resize_y > panel.height) {
                            resize_y = panel.height - panel.minSize;
                        }
                        if (panel.maxSize && (panel.height - resize_y > panel.maxSize)) {
                        ...
                        if (upper_panel.minSize - resize_y > upper_panel.height) {
                            resize_y = upper_panel.minSize - upper_panel.height;
                        }
                        break;
                }
                tmp.diff_x = resize_x;
        ...
        resize: function () {
            ...
            var plleft = this.get('lower_left');
            var pright = this.get('right');
            var plright = this.get('lower_right');
            ...
            var slleft = (plleft !== null && plleft.hidden !== true ? true : false);
            var sright = (pright !== null && pright.hidden !== true ? true : false);
            var slright = (plright !== null && plright.hidden !== true ? true : false);
            ...
            if (pleft !== null && pleft.hidden !== true) {
                l = 0;
                t = 0 + (stop ? ptop.sizeCalculated + (this.resizer > this.padding ? this.resizer : this.padding) : 0);
                w = pleft.sizeCalculated;
                h = height - (stop ? ptop.sizeCalculated + separator : 0) -
                    (slleft ? plleft.sizeCalculated + separator : 0) -
                    (sbottom ? pbottom.sizeCalculated + separator : 0);
                ...
                pleft.height = h;
                // resizer
                h += (slleft ? plleft.sizeCalculated + separator : 0);
            ...
            // lower left if any
            if (plleft !== null && plleft.hidden !== true) {
                l = 0;
                t = height - plleft.sizeCalculated - (sbottom ? pbottom.sizeCalculated + separator : 0);
                w = pleft.sizeCalculated;
                h = plleft.sizeCalculated;
                e = $('#layout_' + this.name + '_panel_lower_left');
                if (window.navigator.userAgent.indexOf('MSIE') != -1 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight) w += 17; // IE hack
                e.css({
                    'display': 'block',
                    'left': l + 'px',
                ...
                h = separator;
                $('#layout_' + this.name + '_resizer_lower_left').show().css({
                    'display': 'block',
                    'left': l + 'px',
                    'top': t + 'px',
                    'width': w + 'px',
                ...
                if (plleft.resizable) {
                    $('#layout_' + this.name + '_resizer_lower_left').show().css({
                        ...
                        // event before
                        var eventData = obj.trigger({ phase: 'before', type: 'resizerClick', target: 'lower_left', originalEvent: event });
                        if (eventData.isCancelled === true) return;
                        // default action
                        w2ui[obj.name].tmp.events.resizeStart('lower_left', event);
                        // event after
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                        return false;
                    });
                ...
                $('#layout_' + this.name + '_panel_lower_left').hide();
                $('#layout_' + this.name + '_resizer_lower_left').hide();
            }
            // right if any
            if (pright !== null && pright.hidden !== true) {
                l = width - pright.sizeCalculated;
                t = 0 + (stop ? ptop.sizeCalculated + separator : 0);
                w = pright.sizeCalculated;
                h = height - (stop ? ptop.sizeCalculated + separator : 0) -
                    (slright ? plright.sizeCalculated + separator : 0) -
                    (sbottom ? pbottom.sizeCalculated + separator : 0);
                ...
                pright.height = h;
                // resizer
                h += (slright ? plright.sizeCalculated + separator : 0);
                l = l - separator;
                w = separator;
            ...
            // lower right if any
            if (plright !== null && plright.hidden !== true) {
                l = width - pright.sizeCalculated;
                //height - (sbottom ? pbottom.sizeCalculated + this.padding : 0) - pprev.sizeCalculated;
                t = height - plright.sizeCalculated - (sbottom ? pbottom.sizeCalculated + separator : 0);
                w = pright.sizeCalculated;
                h = plright.sizeCalculated;
                e = $('#layout_' + this.name + '_panel_lower_right');
                if (window.navigator.userAgent.indexOf('MSIE') != -1 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight) w += 17; // IE hack
                e.css({
                    'display': 'block',
                    'left': l + 'px',
...
//                w = (this.resizer > this.padding ? this.resizer : this.padding);
                $('#layout_' + this.name + '_resizer_lower_right').show().css({
                    'display': 'block',
                    'left': l + 'px',
                    'top': t + 'px',
                    'width': w + 'px',
                ...
                if (plright.resizable) {
                    $('#layout_' + this.name + '_resizer_lower_right').show().css({
                        'cursor': 'ns-resize'
                    }).off('mousedown').on('mousedown', function (event) {
                        // event before
                        var eventData = obj.trigger({ phase: 'before', type: 'resizerClick', target: 'lower_right', originalEvent: event });
                        if (eventData.isCancelled === true) return;
                        // default action
                        w2ui[obj.name].tmp.events.resizeStart('lower_right', event);
                        // event after
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                        return false;
                    });
                ...
                $('#layout_' + this.name + '_panel_lower_right').hide();
                $('#layout_' + this.name + '_resizer_lower_right').hide();
            }
            // bottom if any
            ...
            // main - always there
            l = 0 + (sleft ? pleft.sizeCalculated + separator : 0);
            t = 0 + (stop ? ptop.sizeCalculated + separator : 0);
            w = width - (sleft ? pleft.sizeCalculated + separator : 0) -
                (sright ? pright.sizeCalculated + separator : 0);
            h = height - (stop ? ptop.sizeCalculated + separator : 0) -
                (sbottom ? pbottom.sizeCalculated + separator : 0) -
                (sprev ? pprev.sizeCalculated + separator : 0);
            ...
            if (pprev !== null && pprev.hidden !== true) {
                l = 0 + (sleft ? pleft.sizeCalculated + separator : 0);
                t = height - (sbottom ? pbottom.sizeCalculated + separator : 0) - pprev.sizeCalculated;
                w = width - (sleft ? pleft.sizeCalculated + separator : 0) -
                    (sright ? pright.sizeCalculated + separator : 0);
